...
$connector = new WooEMI_KlicktippConnector();
...
class WooEMI_KlicktippConnector {
  protected $host;
  protected $port;
  protected $path;
  private $sessionId;
  ...
  /**
   * Instantiates a WooEMI_KlicktippConnector
   ...
   */
  public function __construct($service = 'https://www.klick-tipp.com/api') {
...
/*
WooEMI_KlicktippPartnerConnector
...
$connector = new WooEMI_KlicktippPartnerConnector($username, $developer_key, $customer_key);
$subscriber = $connector->subscribe('example@example.com');
$result = $connector->subscriber_update($subscriber->id, array('fieldFirstName' => 'John'));
$result = $connector->unsubscribe('example@example.com');
$result = $connector->subscriber_delete($subscriber->id);
...
class WooEMI_KlicktippPartnerConnector extends WooEMI_KlicktippConnector {
  protected $ciphertext;
  protected $username;

  /**
   * Instantiates a WooEMI_KlicktippPartnerConnector
   * The service URL will be tested: use get_last_error for any errors detected.
   *
   ...
   * login
   *
   * overrrides WooEMI_KlicktippConnector::login
   ...
   * logout
   *
   * overrrides WooEMI_KlicktippConnector::logout
